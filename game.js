...
function parseFormula(inputText) {
    ...
    str = str.replace(/\(([^()]+)\)\^([0-9.]+)/g, 'Math.pow(($1),$2)');
    str = str.replace(/([x0-9.]+)\^([0-9.]+)/g, 'Math.pow($1,$2)');
    str = str.replace(/(?<!Math\.)pow/g, 'Math.pow');
    str = str.replace(/([0-9])([a-z(])/g, '$1*$2');
    str = str.replace(/\)([0-9a-z])/g, ')*$1');
...
function drawStage(camX = canvas.width / 2, camY = canvas.height / 2, zoom = 1) {
    ...
    const gridEndX = canvas.width + 200; 
    const gridStartY = -2000;
    const gridEndY = 4000;

    ctx.beginPath();
    for (let x = originX; x < gridEndX; x += 40) { ctx.moveTo(x, gridStartY); ctx.lineTo(x, gridEndY); }
    for (let x = originX; x > gridStartX; x -= 40) { ctx.moveTo(x, gridStartY); ctx.lineTo(x, gridEndY); }
    for (let y = originY; y < gridEndY; y += 40) { ctx.moveTo(gridStartX, y); ctx.lineTo(gridEndX, y); }
    for (let y = originY; y > gridStartY; y -= 40) { ctx.moveTo(gridStartX, y); ctx.lineTo(gridEndX, y); }
    ctx.stroke();
...
function executeFireShot() {
    ...
    const formulaString = parseFormula(formulaInput.value);
    ...
    function playImpactCinematic(finalX, finalY, onComplete) {
        ...
        function zoomAnimation() {
            frame++;
            currentZoom = 1.0 - (Math.sin((frame / duration) * (Math.PI / 2)) * 0.48);
            ...
            ctx.strokeStyle = '#ff3366'; ctx.lineWidth = 2.5 / currentZoom; 
            ctx.beginPath();
            shotPath.forEach((pt, idx) => {
    ...
    function animate() {
        ...
        if (isNaN(canvasX) || isNaN(canvasY) || !isFinite(canvasX) || !isFinite(canvasY)) {
            errorDisplay.innerText = `[軌道エラー]: 計算不能な数値を検出しました。`;
            isAnimating = false;
            drawStage();
        ...
        if (t === 0) {
            canvas.dataset.camX = canvasX;
            canvas.dataset.camY = canvasY;
        }
        ...
        const easing = 0.15; 
        currentCamX += (canvasX - currentCamX) * easing;
        currentCamY += (canvasY - currentCamY) * easing;
        ...
        ctx.fillStyle = '#fff'; ctx.beginPath(); ctx.arc(canvasX, canvasY, 4, 0, Math.PI * 2); ctx.fill();
        ctx.restore();

        const isOutOfWidth = canvasX > canvas.width + 200 || canvasX < -200;
        const isOutOfHeight = canvasY > canvas.height * 2 || canvasY < -canvas.height * 2;
...
function detectDevice() {
    ...
    if (isTouchDevice) {
        gameContainer.classList.add('hud-touch');
    } else {
        gameContainer.classList.remove('hud-touch');
    }
}
